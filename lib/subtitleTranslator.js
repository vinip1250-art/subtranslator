...
const BLOCK        = 2000;  // chars por chunk (menor = mais seguro contra timeout)
const MAX_CHARS    = 80000; // limite de tamanho: ignora legendas gigantes
const CONCURRENCY  = 3;     // máximo de chamadas paralelas à API
const RETRY_DELAY  = 1500;  // ms entre tentativas
const MAX_RETRIES  = 2;
  ...
  for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
    ...
    try {
      const resp = await fetch(apiUrl, {
        ...
        signal:  AbortSignal.timeout(15000)  // 15s timeout por chunk
      });
...
export async function translateSrt(srtText, from = "en", to = "pt") {
  ...
  // Valida que a URL aponta para o endpoint correto
  const apiUrl = API_URL.endsWith("/translate") ? API_URL : API_URL.replace(/\/$/, "") + "/translate";
  console.log("[subtrans] API endpoint:", apiUrl);

  // Limita tamanho para evitar timeout total da função Vercel (10s no free tier)
  const text = srtText.length > MAX_CHARS ? srtText.substring(0, MAX_CHARS) : srtText;
  if (srtText.length > MAX_CHARS) {
    console.warn(`[subtrans] Legenda truncada de ${srtText.length} para ${MAX_CHARS} chars`);
  }
  ...
  // Traduz com concorrência limitada
